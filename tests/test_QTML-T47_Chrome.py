...
from pages.BICL.account_info.scanned_documents import scanned_documents
...
class Test_login_Chrome:
    ...
    def test_login_chrome(self, browser, env):
        ...
        # Navigate to Account Info Panel
        ...
        ddd.select_scanned_documents()

        time.sleep(10)

        sc = scanned_documents(driver)

        # Click on Address Change Document

        sc.click_address_change_form()

        time.sleep(30)

        # Take screenshot after clicking Scanned Documents and save to utilities/test_results/screenshots
        ...
        if driver.find_element_by_id("errorShowMore").is_displayed():
            errorDisplays = True
        else:
            pass
