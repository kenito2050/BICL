...
from pages.BICL.account.order_inquiry.order_inquiry import order_inquiry
...
class Test_login_Chrome:
    ...
    def test_login_chrome(self, browser, env):
        ...
        # Click Order Inquiry
        a.click_order_inquiry()

        time.sleep(15)

        oi = order_inquiry(driver)

        # Validate T Column
        oi.verify_integers_in_T_column(test_case_ID, browser, env, time_stamp)

        # Validate Quantity Column
        oi.verify_thousands_in_quantity_column(test_case_ID, browser, env, time_stamp)

        # Validate Leaves Column
        oi.verify_integers_in_leaves_column(test_case_ID, browser, env, time_stamp)
