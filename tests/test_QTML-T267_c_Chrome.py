...
from pages.BICL.notes_modal.notes_modal import notes_modal
from pages.BICL.notes_modal.activity_notes_tab.activity_notes_tab import activity_notes_tab
...
class Test_login_Chrome:
    ...
    def test_login_chrome(self, browser, env):
        ...
        # Click Notes Button
        dp.click_notes_button()

        time.sleep(15)

        nm = notes_modal(driver)

        # click on "Account Notes"
        nm.click_activity_notes_tab()

        time.sleep(3)

        # Click PDF Export Button

        acnt = activity_notes_tab(driver)

        acnt.click_export_pdf_button()

        time.sleep(3)

        # Switch to Rep Notes PDF
